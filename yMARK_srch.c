...
yMARK_execute         (uchar *a_search)
{
   ...
   if (strchr (YSTR_SJOIN, x_join) != NULL) {
      ...
      while (x_find != NULL) {
         switch (x_join) {
         case G_CHAR_AND  : case '–' :
            if (x_find->c < 2) {
               DEBUG_SRCH   yLOG_complex ("– remove"  , "%3d#, %2dc %s", n, x_find->c, x_find->label);
               ymark_find_free (&x_find);
            } else  {
               DEBUG_SRCH   yLOG_complex ("– skipped" , "%3d#, %2dc %s", n, x_find->c, x_find->label);
               ++n;
            }
            break;
         case G_CHAR_XOR  : case '“' :
            if (x_find->c > 1) {
               DEBUG_SRCH   yLOG_complex ("“ remove"  , "%3d#, %2dc %s", n, x_find->c, x_find->label);
               ymark_find_free (&x_find);
            } else {
